...
class test_hipotese_scipy:
    def __init__(self,controle, tratamento, TSHN=stats.mannwhitneyu, **kwargs):
        self.controle = controle
        self.tratamento = tratamento
        self.TSHN =  TSHN
        self.kwargs = kwargs
    ...
    def _calc_p(self):
        s, self.p = self.TSHN(self.controle, self.tratamento, **self.kwargs )
        self.p = self.p *100
...
class test_hipotese_scipy_one:
    def __init__(self,controle, TSHN=stats.mannwhitneyu):
        self.controle = controle
        # self.tratamento = tratamento
        self.TSHN =  TSHN
    ...
    def _calc_p(self):
        s, self.p = self.TSHN(self.controle)
        self.p = self.p *100
...
def test_hipotese_kwarg():
    
    controle =   [ 54.9, 128.53333333,  87.86666667,  58.06666667, 109.0, 75.4 ]
    tratamento  =[ 31.46666667,  34.56666667, 119.9,         64.63333333,  87.96666667, 79.46666667]

    t = test_hipotese_scipy(controle, tratamento, stats.levene, center="mean")
    print(t.p)

def test_test_hipotese_scipy_one():
    data = [ 31.46666667,  34.56666667, 119.9       ,  64.63333333,
        87.96666667,  79.46666667]
    s, p = stats.shapiro(data)
    print(s,p)

    th = test_hipotese_scipy_one(data, stats.shapiro)
